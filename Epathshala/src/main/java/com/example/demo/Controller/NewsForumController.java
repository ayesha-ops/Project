...
@Controller
@RequestMapping("/student/news-forum")
public class NewsForumController {
    @Autowired
    ...
    public ModelAndView allForumTopics(@PathVariable String userID) {
        ModelAndView model = new ModelAndView("webpages/forums/allForumTopicsPage");
        model.addObject("username", userID);
        List<Forums> forums = newsForumDAO.getAllForumTopics(userID);
    ...
    public ModelAndView SearchTopic(@PathVariable String userID, HttpServletRequest request) {
        ...
        if (request.getParameter("keyword") != null) {
            forums = newsForumDAO.searchTopics(userID, request.getParameter("keyword"));
        } else {
            forums = newsForumDAO.getAllForumTopics(userID);
        }
        model.addObject("all_topics", forums);
        HashMap<Integer, Integer> map = new HashMap<Integer, Integer>();
        for (Forums f : forums) {
    ...
    @RequestMapping(value = "/view-specific-forum/{userID}/{forumID}", method = RequestMethod.GET)
    public ModelAndView allComments(@PathVariable String userID, @PathVariable String forumID) {
        ModelAndView model = new ModelAndView("webpages/forums/specificForumPage");
        model.addObject("username", userID);
    ...
    public ModelAndView likeComment(@PathVariable String userID, @PathVariable String forumID, @PathVariable String commentID) {
        ...
        return new ModelAndView("redirect:/student/news-forum/view-specific-forum/{userID}/{forumID}");
    }
    ...
    @RequestMapping(value = "/delete-comment/{userID}/{forumID}/{commentID}", method = RequestMethod.GET)
