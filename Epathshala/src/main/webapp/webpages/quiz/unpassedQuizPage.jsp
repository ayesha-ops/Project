...
<nav class="navbar navbar-expand-lg navbar navbar-light" style="background-color: #e3f2fd;">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <a class="nav-link active" aria-current="page" href="/course/topicspage/${userID}/${courseID}" role="button"style="color: black;"><i class="cil-arrow-circle-left"></i></a>
...
<div class="mx-auto" style="width: 1200px;">
    <div class="rounded float-left" style="padding-right: 50px; padding-left: 20px;" width="550" height="300" align="right">
        ...
        <br>
        <h6 style= "color:green">Obtained Highest Marks: ${obtainedMarks}</h6></div>
...
<br><br>
        <form:form action="/course/quiz-page/submit-quiz/${username}/${courseID}" method="post">
            <c:forEach var="i" begin="1" end="${quizes.size()}">
                <div class="question-panel" style="margin-left: 50px">
                    ...
                    <div class="answer">
                        ...
                        <label>
                            <input type="radio" name="ans${i-1}" value="${quizes.get(i-1).option1}" />
                        </label>
                       ${quizes.get(i-1).option1}&nbsp;&nbsp;&nbsp;&nbsp;
                        <label>
                            <input type="radio" name="ans${i-1}" value="${quizes.get(i-1).option2}" />
                        </label>
                            ${quizes.get(i-1).option2}&nbsp;&nbsp;&nbsp;&nbsp;
                        <label>
                            <input type="radio" name="ans${i-1}" value="${quizes.get(i-1).option3}" />
                        </label>
                            ${quizes.get(i-1).option3}&nbsp;&nbsp;&nbsp;&nbsp;
                        <label>
                            <input type="radio" name="ans${i-1}" value="${quizes.get(i-1).option4}" />
                        </label>
                            ${quizes.get(i-1).option4}&nbsp;&nbsp;&nbsp;&nbsp;
<%--                        <form:radiobutton path="ans" name="ans${i}" value="${quizes.get(i-1).option1}" label="${quizes.get(i-1).option1}"/>&nbsp;--%>
<%--                        <form:radiobutton path="ans" name="ans${i}" value="${quizes.get(i-1).option2}" label="${quizes.get(i-1).option2}"/>&nbsp;--%>
<%--                        <form:radiobutton path="ans" name="ans${i}" value="${quizes.get(i-1).option3}" label="${quizes.get(i-1).option3}"/>&nbsp;--%>
<%--                        <form:radiobutton path="ans" name="ans${i}" value="${quizes.get(i-1).option4}" label="${quizes.get(i-1).option4}"/>&nbsp;--%>

<%--                        <input type="hidden" name="question${i-1}" value="${quizes.get(i-1).quiz_ques}">--%>
<%--                        <input type="hidden" name="qid${i-1}" value="${quizes.get(i-1).quiz_id}">--%>
<%--                        <input type="hidden" name="actual_ans${i-1}" value="${quizes.get(i-1).quiz_ans}">--%>
                        <br>
                        <br>
...
<%--            <input type="hidden" name="highest_mark" value="${totalMarks}">--%>
            <button type="submit" value="Submit"  class="btn btn-success mb-3" style="height:40px; width:100px">
                submit</button>
        </form:form>
