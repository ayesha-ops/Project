...
<nav class="navbar navbar-expand-lg navbar navbar-light" style="background-color: #e3f2fd;">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <a class="nav-link active" aria-current="page" href="/admin/${adminID}/showTeachers" role="button"style="color: black;"><i class="cil-arrow-circle-left"></i></a>
...
<div class="mx-auto" style="width: 1100px;">
    <img src="${users.picture}" class="rounded float-left" style="padding-right: 50px; padding-left: 20px;" width="400" height="300" align="right">
    <div class="rounded float-left">
        <h1 class="card-title" style="align: center;color:blue"><i class="fa fa-user" aria-hidden="true"style="font-size:30px; color:blue"></i>  ${users.full_name}</h1><!-write the course_ID->
        <p class="card-text">${teachers.t_designation}</p>
        <p class="card-text">${teachers.t_speciality}</p>

    </div>
    <br>
    <form action="/admin/submit/${adminID}/${userID}/DeleteTeachers" method="post">
        <button type="submit" name="action" value = "deleteTeachers" class="btn btn-lg btn-success" >
    ...
    <c:forEach var="course" items="${createdCourseList}" varStatus="status">
        <h5> <i class="fa fa-graduation-cap" style="font-size:20px;color:red"></i>
            <a href="/admin/${adminID}/${course.course_id}/showCoursePage">
                    ${course.title}</a></h5><hr>
    </c:forEach>
    <hr>
    <br><br>
</div>
